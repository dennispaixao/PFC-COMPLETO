...
const listar = (busca) => {
    ...
    xhr.open('GET', 'BuscarListaEstoqueMateriaisAjax?busca=' + busca);
    ...
    xhr.onload = function () {
        ...
        if (xhr.status === 200) {
            ...
            for (let i = 0; i < produtos.length; i++) {
                ...
                }else if(parseFloat(produtos[i].qtEstoque) >= parseFloat(produtos[i].qtMin)){
                ...
                campoEstRaz.value = produtos[i].qtMin;
                ...
                campoQtEstoque.addEventListener("change", () => {
                    let xhr = new XMLHttpRequest();
                    xhr.open("POST", 'AlterarQtEstoqueMateriaisAjax?id='+idProd+"-"+ campoQtEstoque.value);
                    xhr.onload = function () {
                        //se tudo ocorrer bem como proceder
                        if (xhr.status === 200) {
                            listar("");
                ...
                campoEstRaz.addEventListener("change", () => {
                    let xhr = new XMLHttpRequest();
                    xhr.open("POST", 'AlterarPadraoEstoqueMateriaisAjax?id='+idProd+"-"+ campoEstRaz.value);
                    xhr.onload = function () {
                        //se tudo ocorrer bem como proceder
                        if (xhr.status === 200) {
                            listar("");
